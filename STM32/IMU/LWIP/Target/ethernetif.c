...
static void low_level_init(struct netif *netif)
{
  ...
  memcpy(MACAddr, &eth_addr_config.mac_prefix, 4);
  const uint32_t stm32_uid[3] = {HAL_GetUIDw0(), HAL_GetUIDw1(), HAL_GetUIDw2()};
  stm32_uid_hash = crc32((uint8_t*)stm32_uid, sizeof(stm32_uid));
  MACAddr[4] = (stm32_uid_hash >> 8) & 0xFF;
  MACAddr[5] = (stm32_uid_hash >> 0) & 0xFF;

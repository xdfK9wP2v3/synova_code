...
class Params:
    ...
    # IMU Parameters
    g_n = np.array([0., 0., 9.79469967])
    acc_cov: np.ndarray = np.eye(3) * (2.3 * 9.8 * 1e-3) ** 2                   #
    gyr_cov: np.ndarray = np.eye(3) * (0.17 / 180 * np.pi) ** 2        #

    # Magnetometer Parameters
    # Magnetic declination: deg (W: -1, E: +1)
    mag_var = 4.6  # @@ should be updated with RTK data

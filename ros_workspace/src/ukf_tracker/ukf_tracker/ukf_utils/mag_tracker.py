...
class MAGTracker(Tracker):
    ...
    def mag_update(self, data: Mag, step_end: bool = True) -> None:
        xs, ys, wm, wc = self.unscented_mag_measurement(self.state.m.value, self.state.P.value)
        mea: np.ndarray = np.array([data.mag.x, data.mag.y, data.mag.z])
        ...
        if step_end:
            ...
            self.save_step(current_time=ts, sensor_type='MAG')
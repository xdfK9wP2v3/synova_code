...
public:
    ...
    uint32_t Get_wait(T *buffer, const uint32_t len ,const uint32_t timeout_ms) {
        uint32_t cnt = 0;
        while (length() < len) {
            if(cnt++>=timeout_ms)
              break;
        ...
        // std::cout << "[UnlockQueue] length(): " << length() << std::endl;
        return Get(buffer, len);
    }

    uint32_t size() { return _size; }
    uint32_t length() { return (_in - _out); }
    bool empty() { return _in <= _out; }
